    ...
    if (!allArr.length) {
        ...
        for (var i = 0; i < allArr.length; i++) {
            ...
            var dom = $('<tr class="small">' +
                ...
                '<td class="border"><input type="text" placeholder="填写" data-type="remark" value=' + (item.remark || '无') + '></td>' +
                '<td class="border"><a href="javascript:;" class="delete-hover">删除</a></td>' +
                '</tr>');
            dom.data('item', item);
            tbody.append(dom);
...
exports.renderBidsInvitationTable = function (arr) {
    ...
    for (var i = 0; i < arr.length; i++) {
        ...
        var dom = $('<tr class="small">' +
            ...
            '<td class="border" style="position: relative;"><a href="javascript:;" class="confirm-hover" data-type="check">查看</a></td>' +
...
exports.prevNewBid = function (data) {
    ...
    if (data.bidType == 1) { // 材料
        for (var i = 0; i < data._addBidDetailList.length; i++) {
            ...
            var dom = $('<tr class="small">' +
                ...
                '<td class="border">' + (item.remark || '无') + '</td>' +
                '</tr>');
    ...
    if(data._addBidDetailList.length){
        $('.bidListPrev-container').show();
    } else {
        $('.bidListPrev-container').hide();
    }
    /* 投标邀请 */
...
exports.renderCompanyInfoTable = function (data, modal) {
    ...
    if(!data.attachList) {
        ...
        for (var i = 0; i < data.attachList.length; i++) {
            ...
            var dom = $('<div class="fl" style="width: 110px;height: 90px;padding: 3px;border: 1px solid #e5e5e5;margin: 0 10px 10px 0;border-radius: 3px;cursor: pointer;">' +
                '<a target="_blank" href='+attachUrl+' style="display: block;width: 102px;height: 82px;background-size: cover;background-repeat: no-repeat;background-image: url(' + attachUrl + ')"></a>' +
                '</div>');
...
exports.renderBidingInfoList = function (data, modal) {
    ...
    if(list.length){
        modal.$body.find('.table-content').show();
        modal.$body.find('.bid-remark-container').hide();
        for (var i = 0; i < list.length; i++) {
            var item = list[i];
            var dom = $('<tr class="small">' +
                '<td class="border">' + (i + 1) + '</td>' +
        ...
        modal.$body.find('.table-content').hide();
        modal.$body.find('.bid-remark-container').show();
        modal.$body.find('.bid-remark').html(data.biddingRemark);
    }
}
...
* 中标详情
* */
exports.renderBidNoticeList = function (data) {
    $("#bidRequireList").html('');
    $('#bidDetailVOList').html('');
    for (var i = 0; i < data.bidRequireList.length; i++) {
        var item = data.bidRequireList[i];
        var dom = $('<tr class="small">' +
            '<td class="border">' + (i + 1) + '</td>' +
            '<td class="border">' + item.requireDesc + '</td>' +
            '</tr>');
    ...
    if(data.bidDetailVOList.length){ // 非企业服务
        ...
        for (var i = 0; i < data.bidDetailVOList.length; i++) {
            var item = data.bidDetailVOList[i];
            var dom = $('<tr class="small">' +
                '<td class="border">' + (i + 1) + '</td>' +
                '<td class="border">' + item.objName + '</td>' +
                '<td class="border">' + item.objContent + '</td>' +
        ...
        $('.bidDetailVOList-container').hide();
        $('.bid-remark-container').show();
        $('.bid-remark').html(data.biddingRemark);
    }
}